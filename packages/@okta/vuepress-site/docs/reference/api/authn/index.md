...
> **Note:** The use of the `X-Device-Fingerprint` header for new device security behavior detection is deprecated. Starting April 12 2021, we are going to enable [improvements to the new device security behavior](https://help.okta.com/en/prod/Content/Topics/Security/improved-new-device-behavior-detection.htm#:~:text=Improved%20New%20Device%20Behavior%20Detection%20supports%20browsers%20that%20store%20HTTP,end%2Duser%20activity%20and%20behavior) for all the existing tenants. Once the improvements are rolled out, new device security behavior would only rely on the `deviceToken` in the [Context Object](#context-object) and would not rely on the `X-Device-Fingerprint` header.
